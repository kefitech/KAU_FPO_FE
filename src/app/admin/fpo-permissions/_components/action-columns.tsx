...
interface ActionCallbacks {
  onEdit: (action: FpoAction) => void;
  onToggleActive: (action: FpoAction) => void;
  onDelete: (action: FpoAction) => void;
}

function TranslationsCell({ action, t }: { action: FpoAction; t: T }) {
  const router = useRouter();
  const langs = action.translations as string[];

  function handleClick(e: React.MouseEvent) {
    e.stopPropagation();
    router.push(`/admin/languages?tab=translations&category=10&search=${action.code}`);
  }
...
function FpoActionRowActions({
  ...
  return (
    <RowActions
      actions={[
        { label: tCommon.edit ?? "Edit", onClick: () => callbacks.onEdit(action) },
        {
          label: action.is_active ? (t.action_deactivate ?? "Deactivate") : (t.action_activate ?? "Activate"),
          onClick: () => callbacks.onToggleActive(action),
        },
        {
          label: tCommon.delete_btn ?? "Delete",
          onClick: () => callbacks.onDelete(action),
          destructive: true,
          separator: true,
        },
      ]}
...
export function getActionColumns(t: T = {}, tCommon: T = {}, callbacks: ActionCallbacks): ColumnDef<FpoAction>[] {
  return [
    {
      accessorKey: "code",
    ...
    },
    {
      ...
      enableSorting: false,
      cell: ({ row }) => <TranslationsCell action={row.original} t={t} />,
    },
    {
    ...
    },
    {
      id: "actions",
      header: "",
      cell: ({ row }) => <FpoActionRowActions action={row.original} t={t} tCommon={tCommon} callbacks={callbacks} />,
    },
  ];
}
