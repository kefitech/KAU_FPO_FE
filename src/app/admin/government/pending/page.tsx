...
export default function PendingGovernmentApprovalsPage() {
  ...
  useEffect(() => {
    translationsApi
      .getPublic(locale, "government_pending")
      .then((data) => setT(data.government_pending ?? {}))
      .catch(() => undefined);
  }, [locale]);
  ...
  return (
    <div className="flex flex-col gap-6 py-6">
      ...
      >
        <ChevronLeft className="h-4 w-4" /> {t.back_button ?? "Back to Government Officials"}
      </button>

      <div>
        <h1 className="font-bold text-2xl">{t.page_title ?? "Pending Approvals"}</h1>
        <p className="mt-0.5 text-muted-foreground text-sm">
          {t.page_description ?? "Self-registered government officials awaiting review"}
        </p>
      </div>
