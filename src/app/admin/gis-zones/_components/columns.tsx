...
function StatusAndActions({ version, t }: { version: ZoneVersion; t: T }) {
  const queryClient = useQueryClient();
  const confirm = useConfirmStore((s) => s.confirm);

  const activateMutation = useMutation({
    mutationFn: () => adminGisZonesApi.activate(version.id),
    onSuccess: (result) => {
      toast.success((t.toast_activated ?? "Zones updated: {zones}").replace("{zones}", result.updated_zones.join(", ")));
      queryClient.invalidateQueries({ queryKey: ["gis-zone-versions"] });
      queryClient.invalidateQueries({ queryKey: ["gis-zones-map"] });
    },
  ...
  const deleteMutation = useMutation({
    mutationFn: () => adminGisZonesApi.delete(version.id),
    onSuccess: () => {
...
export function getZoneVersionColumns(t: T): ColumnDef<ZoneVersion>[] {
  return [
    {
      accessorKey: "label",
