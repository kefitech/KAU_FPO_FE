...
function EmailOtpVerifyBlock({
  ...
  return (
    <div>
      <h3 className="font-semibold text-base">{t.otp_email_title ?? "Verify Email Address"}</h3>
      <p className="mt-1 text-muted-foreground text-sm">
        {t.otp_email_desc ?? "We'll send a one-time password to confirm your email address."}
      </p>
      ...
      <div className="mt-4 flex flex-col gap-2">
        ...
        <div className="flex flex-col gap-2 sm:flex-row">
          <Input
            ...
            placeholder={t.otp_email_placeholder ?? "you@example.com"}
            disabled={otpSent}
            className="flex-1"
          />
          {!otpSent && (
            ...
            >
              {sendMutation.isPending ? (t.otp_sending_btn ?? "Sending…") : (t.otp_send_email_btn2 ?? "Send OTP")}
            </Button>
          )}
        </div>
        {otpSent && (
...
function PhoneOtpVerifyBlock({
  ...
  return (
    <div>
      <h3 className="font-semibold text-base">{t.otp_phone_title ?? "Verify Phone Number"}</h3>
      <p className="mt-1 text-muted-foreground text-sm">
        {t.otp_phone_desc ?? "We'll send a one-time password to confirm your mobile number."}
      </p>
      ...
      <div className="mt-4 flex flex-col gap-2">
        ...
        <div className="flex flex-col gap-2 sm:flex-row">
          <Input
            ...
            placeholder={t.otp_phone_placeholder ?? "10-digit mobile number"}
            disabled={otpSent}
            className="flex-1"
          />
          {!otpSent && (
            <Button
              ...
              className="bg-green-600 hover:bg-green-700 sm:w-auto"
            >
              {sendMutation.isPending ? (t.otp_sending_btn ?? "Sending…") : (t.otp_send_btn2 ?? "Send OTP")}
            </Button>
          )}
        </div>
        {otpSent && (
...
function StepIndicator({ currentStep, t }: { currentStep: 1 | 2; t: T }) {
  const steps = [
    { num: 1 as const, label: t.step1_label ?? "Verify Contact" },
    { num: 2 as const, label: t.step2_label ?? "Account Details" },
  ];
  return (
      ...
      {steps.map((step, i) => (
        <div key={step.num} className="flex items-center gap-2">
          <div className="flex items-center gap-2">
            ...
            <span
              className={`text-sm ${step.num === currentStep ? "font-medium text-foreground" : "text-muted-foreground"}`}
            >
              {step.label}
            </span>
          </div>
              ...
              <Card>
                ...
                <CardContent className="flex flex-col gap-4">
                  ...
                  <div className="rounded-lg border bg-muted/30 p-3">
                    <p className="text-muted-foreground text-xs">{t.field_email ?? "Email"}</p>
                    <p className="font-medium text-sm">{form.watch("email")}</p>
                  </div>
                  <div className="rounded-lg border bg-muted/30 p-3">
                    <p className="text-muted-foreground text-xs">{t.field_phone ?? "Phone"}</p>
                    <p className="font-medium text-sm">{form.watch("phone")}</p>
                  </div>
                  <Controller
                    control={form.control}
                    name="designation"
