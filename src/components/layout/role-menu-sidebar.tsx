...
interface RoleMenuSidebarProps {
  title: string;
  subtitle: string;
  /** Only pass this when the portal has a dedicated settings page outside the fetched menu (e.g. admin). */
  settingsHref?: string;
}

export function RoleMenuSidebar({ title, subtitle, settingsHref }: RoleMenuSidebarProps) {
    ...
    <Sidebar collapsible="icon">
      ...
      <SidebarFooter className="border-border/50 border-t pt-2">
        ...
        {isLoading ? (
          ...
          <SidebarMenuItem>
            {settingsHref ? (
              <SidebarMenuButton asChild isActive={pathname.startsWith(settingsHref)}>
                <Link
                  href={settingsHref}
